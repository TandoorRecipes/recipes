...
!!! info "Import Format"
    ...
    When using the Default integration, imports expect the exported archive (`.zip`) generated by Tandoor.

    Importing only the extracted `.json` file is not supported and may result in an import error.

    To migrate recipes between Tandoor instances, use the exported ZIP archive directly without extracting its contents.
