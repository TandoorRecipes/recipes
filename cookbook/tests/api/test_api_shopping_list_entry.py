...
def test_update(arg, request, sle):
    ...
    r = c.patch(reverse(DETAIL_URL, args={sle[0].id}), {'amount': new_val}, content_type='application/json')
...
def test_add(arg, request, sle):
    c = request.getfixturevalue(arg[0])
    r = c.post(reverse(LIST_URL), {'food': {'id': sle[0].food.__dict__['id'], 'name': sle[0].food.__dict__['name'], }, 'amount': 1}, content_type='application/json')
    response = json.loads(r.content)
    print(r.content)
    assert r.status_code == arg[1]
...
@pytest.mark.parametrize("shared, count, sle_2", [('g1_s1', 20, {
    'user': 'g1_s1'
}), ('g1_s2', 10, {
    'user': 'g1_s2'
}), ('u2_s1', 20, {
...
def test_sharing(request, shared, count, sle_2, sle, u1_s1):
    ...
    x = shared_client.patch(reverse(DETAIL_URL, args={sle[0].id}), {'checked': True}, content_type='application/json')
...
def test_auto_onhand(sle, u1_s1):
    user = auth.get_user(u1_s1)
    user.userpreference.shopping_recent_days = 7  # hardcoded API limit 14 days
    user.userpreference.save()
