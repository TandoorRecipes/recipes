...
@pytest.mark.parametrize("use_mealplan", [
    (False),
    (True),
])
def test_shopping_recipe_edit(request, recipe, sle_count, use_mealplan, u1_s1, u2_s1):
    ...
    with scopes_disabled():
        ...
        if use_mealplan:
            ...
            u2_s1.put(reverse(SHOPPING_RECIPE_URL, args={recipe.id}), {'list_recipe': list_recipe, 'servings': 2 * recipe.servings}, content_type='application/json')
        r = json.loads(u1_s1.get(reverse(SHOPPING_LIST_URL)).content)
        ...
        if use_mealplan:
            ...
            u1_s1.put(reverse(SHOPPING_RECIPE_URL, args={recipe.id}), {'list_recipe': list_recipe, 'servings': .5 * recipe.servings}, content_type='application/json')
        r = json.loads(u1_s1.get(reverse(SHOPPING_LIST_URL)).content)
        ...
        u2_s1.put(reverse(SHOPPING_RECIPE_URL, args={recipe.id}), {'list_recipe': list_recipe, 'ingredients': keep_ing}, content_type='application/json')
        r = json.loads(u1_s1.get(reverse(SHOPPING_LIST_URL)).content)
        ...
        u2_s1.put(reverse(SHOPPING_RECIPE_URL, args={recipe.id}), {'list_recipe': list_recipe, 'ingredients': all_ing}, content_type='application/json')
        r = json.loads(u1_s1.get(reverse(SHOPPING_LIST_URL)).content)
        assert sum([x['amount'] for x in r]) == amount_sum * .5
        assert len(r) == sle_count
        assert len(json.loads(u2_s1.get(reverse(SHOPPING_LIST_URL)).content)) == sle_count
...
def test_shopping_recipe_userpreference(recipe, sle_count, use_mealplan, user2):
    with scopes_disabled():
        ...
        if use_mealplan:
            MealPlanFactory(space=recipe.space, created_by=user, servings=recipe.servings, recipe=recipe)
