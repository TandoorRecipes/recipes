<template>
  <div class="step" :class="stepClassName">
    ...
    <div class="row mb-1" v-if="recipe.steps.length > 1">
      <div class="col col-md-8">
        <h5 class="step__name text-primary" :class="stepClassNameType">
          ...
          <small
            ...
            class="step__time text-muted"
            v-if="step.time !== 0"
            ><i class="fas fa-user-clock"></i> {{ step_time }}</small
          >
          <small v-if="start_time !== ''" class="step__start-time d-print-none">
            <b-link
              :id="`id_reactive_popover_${step.id}`"
              @click="openPopover"
              href="#"
            >
              {{
      ...
      <div class="col col-md-4 text-right">
        <b-button
          ...
          :class="{
            'step__button-collapse_visible text-primary': details_visible,
            'step_button-collapse_visible_false text-success': !details_visible,
          }"
        >
    ...
    <b-collapse
      ...
      :class="[
        details_visible
          ? 'step__details_visible'
          : 'step__details_visible_false',
      ]"
    ...
    >
      <div class="row">
        <!-- ingredients table -->
        <div
          class="step__ingredients col col-md-4"
          v-if="
            step.show_ingredients_table &&
            step.ingredients.length > 0 &&
            (recipe.steps.length > 1 || force_ingredients)
          "
        >
        ...
        <div
          class="step__instructions col"
          :class="{
            'col-md-8 col-12': recipe.steps.length > 1,
            'col-md-12 col-12': recipe.steps.length <= 1,
          }"
        >
          <!-- step text -->
          <div class="row">
            <div class="col col-md-12">
              <compile-component
                :code="step.instructions_markdown"
                :ingredient_factor="ingredient_factor"
              ></compile-component>
            </div>
          </div>
          ...
          <div class="step__file row" v-if="step.file !== null">
            <div class="col col-md-12">
              <template>
                <div v-if="step.file.preview !== ''">
                  <b-img :src="step.file.preview" fluid-grow></b-img>
                </div>
                <div v-else>
                  <a
                    :href="step.file.file_download"
                    target="_blank"
            ...
            >
              <div class="card-body">
                <h2 class="card-title">
                  <a
                    :href="
                      resolveDjangoUrl('view_recipe', step.step_recipe_data.id)
                    "
                    >{{ step.step_recipe_data.name }}</a
                  >
                </h2>
                <div
                  v-for="(sub_step, index) in step.step_recipe_data.steps"
      ...
      >
        ...
        <div class="row" style="margin-top: 1vh">
          <div class="col-12" style="text-align: right">
            ...
            <b-button @click="updateTime" size="sm" variant="primary">{{
              $t("Ok")
            }}</b-button>
          </div>
        </div>
