  ...
  actions: {
    ...
    updateDeviceSettings: function () {
      localStorage.setItem(
        _LS_DEVICE_SETTINGS,
        JSON.stringify(this.device_settings),
      );
    },
    ...
    loadUserSettings: function (allow_cached_results) {
      ...
      if (s !== null) {
        ...
        console.log(
          `loaded local user settings age ${(new Date().getTime() - this.user_settings.locally_updated_at) / 1000} `,
        );
      }
      if (
        new Date().getTime() - this.user_settings.locally_updated_at >
          _STALE_TIME_IN_MS ||
        ...
        apiClient
          .retrieveUserPreference(localStorage.getItem("USER_ID"))
          .then((r) => {
            for (s in r.data) {
              if (!(s in this.user_settings) && s !== "user") {
                // dont load new keys if no default exists (to prevent forgetting to add defaults)
                console.error(
                  `API returned UserPreference key "${s}" which has no default in UserPreferenceStore.user_settings.`,
                );
              } else {
            ...
            localStorage.setItem(
              _LS_USER_SETTINGS,
              JSON.stringify(this.user_settings),
            );
          })
          .catch((err) => {
    ...
    updateUserSettings: function () {
      let apiClient = new ApiApiFactory();
      apiClient
        .partialUpdateUserPreference(_USER_ID, this.user_settings)
        .then((r) => {
        ...
        .catch((err) => {
          this.currently_updating = false;
          StandardToasts.makeStandardToast(
            this,
            StandardToasts.FAIL_UPDATE,
            err,
          );
        });
    },
    ...
    isStale() {
      return (
        ...
        new Date() - this.updated_at > _STALE_TIME_IN_MS
      );
    },
    /**
