<template>
  <div class="keywords" v-if="props.keywords">
    <slot name="prepend"></slot>

    <v-chip
      class="me-1 mb-1 keyword"
      :class="keywordClass(k.label)"
      :label="props.label"
      :color="props.color"
      :size="props.size"
      :variant="props.variant"
      v-for="k in keywords"
      :to="useUserPreferenceStore().isAuthenticated ? { name: 'SearchPage', query: { keywords: k.id } } : undefined"
    >
      {{ k.label }}
    </v-chip>
...
const keywordClass = (label: string) => "keyword--" + escapeCSS(label)
</script>
