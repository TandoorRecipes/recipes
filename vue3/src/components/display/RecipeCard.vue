<template>
  <template v-if="!props.loading">
    <router-link :to="{ name: 'RecipeViewPage', params: { id: props.recipe.id } }" :target="linkTarget">
      <recipe-image :style="{ height: props.height }" :recipe="props.recipe" rounded="lg" class="mr-3 ml-3"> </recipe-image>
    </router-link>
    <div class="ml-3">
      <div class="d-flex">
        <div class="flex-grow-1 cursor-pointer" @click="openRecipe()">
          <p class="font-weight-bold mt-2 recipe__title">{{ props.recipe.name }}</p>
        </div>
        <div class="mt-1">
          <!--                    <v-btn icon="fas fa-ellipsis-v" size="small" variant="plain"></v-btn>-->
          <recipe-context-menu class="recipe__actions" :recipe="props.recipe" size="small" v-if="props.showMenu"></recipe-context-menu>
        </div>
      </div>
      <!--            <p class="text-disabled">{{ props.recipe.createdBy.displayName}}</p>-->
      <keywords-component class="recipe__keywords" variant="outlined" :keywords="props.recipe.keywords" :max-keywords="3" v-if="props.showKeywords">
        <template #prepend>
          <v-chip class="mb-1 me-1 keyword--private" size="x-small" label variant="outlined" v-if="recipe._private">
            <private-recipe-badge :show-text="false"></private-recipe-badge>
          </v-chip>
          <v-chip class="mb-1 me-1 keyword keyword--external" size="x-small" label variant="outlined" color="info" v-if="props.recipe.internal == false">
            {{ $t("External") }}
          </v-chip>
          <v-chip
            :class="'mb-1 me-1 keyword keyword--worktime-' + props.recipe.workingTime"
            size="x-small"
    ...
    <v-card :to="{ name: 'RecipeViewPage', params: { id: props.recipe.id } }" :style="{ height: props.height }" v-if="false">
      ...
      >
        <template v-slot:activator="{ props }">
          <recipe-image height="70%" width="100%" class="recipe__image" :recipe="props.recipe"> </recipe-image>

          <v-divider class="p-0" v-if="props.recipe.image == null"></v-divider>
        </template>
        <div class="recipe__description" v-if="props.recipe.description != null && props.recipe.description != ''">
          {{ props.recipe.description }}
        </div>
      </v-tooltip>
      <v-card-item>
        <div class="text-rows-2 recipe__title">
