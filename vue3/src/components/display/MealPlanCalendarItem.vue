<template>
  <v-card
    class="card cv-item pa-0 mealplan__item"
    hover
    :style="{ top: itemTop, height: itemHeight, 'border-color': mealPlan.mealType.color }"
    :draggable="true"
    :key="value.id"
    @dragstart="emit('onDragStart', value, $event)"
    :class="[value.classes, escapeCSS('mealtype--' + mealPlan.mealType.name)]"
  >
    <v-card-text class="pa-0">
      <div class="d-flex flex-row align-items-center">
        <div class="flex-column recipe__image" v-if="detailedItems">
          <recipe-image :height="itemHeight" :width="itemHeight" :recipe="mealPlan.recipe"></recipe-image>
        </div>
        <div class="flex-column flex-grow-0 pa-1 recipe__title">
