  ...
  <v-table density="compact" class="ingredients">
    <tbody>
      <template v-for="i in ingredients" :key="i.id">
        <tr @click="i.checked = !i.checked" :class="i.isHeader ? 'ingredients__header' : 'ingredient'">
          <template v-if="i.isHeader">
            <td colspan="5" class="font-weight-bold">{{ i.note }}</td>
          </template>
          <template v-else>
            <td class="pa-0 d-print-none ingredient__check" v-if="showCheckbox">
              <v-checkbox-btn v-model="i.checked" color="success" v-if="!i.isHeader" :class="i.checked ? 'checked--true' : 'checked--false'"></v-checkbox-btn>
            </td>
            <td class="ingredient__amount" :class="i.noAmount ? 'amount--empty' : ''">
              <span
                v-html="calculateFoodAmount(i.amount, ingredientFactor, useUserPreferenceStore().userSettings.useFractions)"
                :class="ingredientClasses[i.id]?.amount"
                v-if="!i.noAmount"
              ></span>
            </td>
            <td class="pr-1 ingredient__unit" :class="i.noAmount ? 'unit--empty' : ''">
              <span v-if="i.unit && !i.noAmount" :class="ingredientClasses[i.id]?.unit">
                {{ ingredientToUnitString(i, ingredientFactor) }}
              </span>
            </td>
            <td class="ingredient__food">
              <template v-if="i.food">
                <span :class="ingredientClasses[i.id]?.food">
                  ...
                  <template v-else>{{ ingredientToFoodString(i, ingredientFactor) }}</template>
                </span>
            ...
            <td class="d-print-none ingredient__note">
...
const tableHeaders = computed(() => {
  ...
  if (props.showNotes) {
    headers.push({ title: "", key: "note", align: "end" })
  }
...
const ingredientClasses = computed(() => {
  const classes: Record<string | number, Record<string, unknown>> = {}
  ingredients.value.forEach((i) => {
    let dict: Record<string, unknown> = {}
    if (i.amount) {
      let amount = calculateFoodAmount(i.amount, props.ingredientFactor, false)
      dict["amount"] = escapeCSS("amount--" + String(amount))
    }
    if (i.unit) {
      dict["unit"] = escapeCSS("unit--" + i.unit.name)
    }
    if (i.food) {
      dict["food"] = escapeCSS("food--" + i.food.name)
    }
    classes[i.id as unknown as string] = dict
  })
  return classes
})
</script>
...
<style scoped>
.ingredient td:not(.ingredient__food) {
  width: 1%;
  text-wrap: nowrap;
}
</style>
