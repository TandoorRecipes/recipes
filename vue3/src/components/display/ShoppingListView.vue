<template>
  <v-tabs class="shopping__tabs" v-model="currentTab">
    <v-tab value="shopping"
      ><i class="fas fa-fw" :class="{ 'fa-circle-notch fa-spin': useShoppingStore().currentlyUpdating, 'fa-shopping-cart ': !useShoppingStore().currentlyUpdating }"></i>
      <span class="d-none d-md-block ms-1">{{ $t("Shopping_list") }} ({{ useShoppingStore().stats.countUnchecked }})</span></v-tab
    >
    <v-tab value="recipes"
      ><i class="fas fa-book fa-fw"></i> <span class="d-none d-md-block ms-1">{{ $t("Recipes") }} ({{ useShoppingStore().getAssociatedRecipes().length }})</span></v-tab
    >
    <v-tab value="selected_supermarket" v-if="useUserPreferenceStore().deviceSettings.shopping_selected_supermarket != null">
      ...
      <span class="d-none d-md-block ms-1">{{ useUserPreferenceStore().deviceSettings.shopping_selected_supermarket.name }}</span>
    </v-tab>

    <v-menu :close-on-content-click="false">
      <template v-slot:activator="{ props }">
        <v-btn class="me-4 float-right" height="100%" rounded="0" variant="plain" v-bind="props">
      ...
      <v-list density="compact">
        ...
        <v-divider></v-divider>
        <v-list-item>
          <v-select hide-details :items="groupingOptionsItems" v-model="useUserPreferenceStore().deviceSettings.shopping_selected_grouping" :label="$t('GroupBy')"> </v-select>
        </v-list-item>
        <v-list-item v-if="useUserPreferenceStore().deviceSettings.shopping_selected_grouping == ShoppingGroupingOptions.CATEGORY">
          <v-switch
            color="primary"
            hide-details
            :label="$t('SupermarketCategoriesOnly')"
            v-model="useUserPreferenceStore().deviceSettings.shopping_show_selected_supermarket_only"
          ></v-switch>
        </v-list-item>
        <v-list-item>
  ...
  <v-window v-model="currentTab">
    <v-window-item class="shopping__list" value="shopping">
      <v-container>
          ...
          <v-col class="pa-0">
            <v-chip-group v-model="useUserPreferenceStore().deviceSettings.shopping_selected_supermarket" v-if="supermarkets.length > 0">
              <v-chip
                ...
                variant="outlined"
                color="primary"
                :class="escapeCSS('supermarket--' + s.name)"
                class="shopping__supermarket"
                >{{ s.name }}</v-chip
              >
            </v-chip-group>
          </v-col>
        ...
        <v-row>
          <v-col>
            <v-alert v-if="useShoppingStore().hasFailedItems()" color="warning" class="mb-2 shopping__warning">
              <template #prepend>
            ...
            <shopping-list-entry-input class="shopping__ing-input"></shopping-list-entry-input>
            ...
            <v-list class="mt-3" density="compact" v-else>
              <template v-for="category in useShoppingStore().getEntriesByGroup" :key="category.name">
                <template v-if="isShoppingCategoryVisible(category)">
                  <v-list-subheader class="shopping__category category--empty" v-if="category.name === useShoppingStore().UNDEFINED_CATEGORY"
                    ><i>{{ $t("NoCategory") }}</i></v-list-subheader
                  >
                  <v-list-subheader class="shopping__category" :class="escapeCSS('category--' + category.name)" v-else>{{ category.name }}</v-list-subheader>
                  <v-divider></v-divider>

                  <template v-for="[i, value] in category.foods" :key="value.food.id">
                    <shopping-line-item class="shopping__item item" :shopping-list-food="value"></shopping-line-item>
                  </template>
                </template>
              </template>
            </v-list>
        ...
        <v-row v-if="useUserPreferenceStore().deviceSettings.shopping_show_debug">
          ...
          <v-col cols="12" md="4">
            <v-card>
              ...
              <v-card-text>
                <v-list>
                  <v-list-item v-for="i in useShoppingStore().undoStack" :key="i">{{ i.type }} {{ i.entries.flatMap((e: ShoppingListEntry) => e.food.name) }} </v-list-item>
                </v-list>
              </v-card-text>
            </v-card>
          </v-col>
    ...
    <v-window-item class="shopping_recipes" value="recipes">
      <v-container>
        <v-row>
          <v-col>
            <v-card>
              ...
              <v-card-text>
                <ModelSelect model="Recipe" v-model="manualAddRecipe" append-to-body>
                  <template #append>
                    <v-btn class="shopping__rcp-input" icon="$create" color="create" :disabled="manualAddRecipe == undefined">
                      <v-icon icon="$create"></v-icon>
                      <add-to-shopping-dialog
                        :recipe="manualAddRecipe"
                        v-if="manualAddRecipe != undefined"
                        @created="useShoppingStore().refreshFromAPI(), (manualAddRecipe = undefined)"
                      ></add-to-shopping-dialog>
                    </v-btn>
                  </template>
                </ModelSelect>
                ...
                <v-list>
                  <v-list-item v-for="r in useShoppingStore().getAssociatedRecipes()">
                    <template #prepend>
                      <v-btn class="recipe__servings" :class="'servings--' + String(r.servings)" color="edit" icon>
                        {{ r.servings }}
                    ...
                    <div class="ms-2">
                      <p class="recipe__title" v-if="r.recipe">{{ r.recipeData.name }}<br /></p>
                      <p v-if="r.mealplan">
                        <span class="mealplan__mealtype" :class="escapeCSS('mealtype--' + r.mealPlanData.mealType.name)">{{ r.mealPlanData.mealType.name }}</span> -
                        {{ DateTime.fromJSDate(r.mealPlanData.fromDate).toLocaleString(DateTime.DATE_FULL) }} #{{ r.id }}
                      </p>
                    </div>
                    ...
                    <template #append>
                      <v-btn icon color="delete">
                        ...
                        <delete-confirm-dialog :object-name="r.name" :model-name="$t('ShoppingListRecipe')" @delete="deleteListRecipe(r)"></delete-confirm-dialog>
                      </v-btn>
                    </template>
                  </v-list-item>
                </v-list>
    ...
    <v-window-item value="selected_supermarket">
      <v-container>
        <v-row>
          <v-col>
            <supermarket-editor
              :item="useUserPreferenceStore().deviceSettings.shopping_selected_supermarket"
...
onMounted(() => {
  ...
  if (useUserPreferenceStore().deviceSettings.shopping_selected_supermarket != null) {
    new ApiApi()
      .apiSupermarketRetrieve({ id: useUserPreferenceStore().deviceSettings.shopping_selected_supermarket!.id! })
      .then((r) => {
...
function updateRecipeServings(recipe: ShoppingListRecipe, servings: number) {
  ...
  api
    .apiShoppingListRecipeUpdate({ id: recipe.id!, shoppingListRecipe: recipe })
    .then((r) => {
...
function deleteListRecipe(slr: ShoppingListRecipe) {
  ...
  api
    .apiShoppingListRecipeDestroy({ id: slr.id! })
    .then((r) => {
      useShoppingStore().refreshFromAPI()
