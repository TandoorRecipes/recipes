<template>
  <v-list-item
    class="swipe-container border-t-sm"
    :id="itemContainerId"
  ...
  >
    ...
    <div class="flex-grow-1 p-2" @click="dialog = true">
      <div class="d-flex">
        <div class="d-flex flex-column pr-2 item__quantity">
    ...
    <template v-slot:[checkBtnSlot]>
      <v-btn
        color="success"
        @click.native.stop="useShoppingStore().setEntriesCheckedState(entries, !isChecked, true)"
    ...
    if (isEntryVisible(e, useUserPreferenceStore().deviceSettings)) {
      ...
      if (e.listRecipe != null) {
        ...
        if (e.listRecipeData.mealplan != null) {
          let meal_plan_entry =
            (e.listRecipeData.mealPlanData.mealType.name.substring(0, 8) || "") +
            (e.listRecipeData.mealPlanData.mealType.name.length > 8 ? ".." : "") +
            " (" +
            DateTime.fromJSDate(e.listRecipeData.mealPlanData.fromDate).toLocaleString(DateTime.DATE_SHORT) +
            ")"
...
function setFoodIgnoredAndChecked(food: Food) {
  ...
  api
    .apiFoodUpdate({ id: food.id!, food: food })
    .then((r) => {})
    .catch((err) => {
