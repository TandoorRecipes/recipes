    ...
    <v-card-text v-if="!isSpaceAtRecipeLimit(useUserPreferenceStore().activeSpace)">
      <v-tabs-window v-model="tab">
        ...
        <v-tabs-window-item value="settings">
          <v-form :disabled="loading || fileApiLoading">
            <v-checkbox :label="$t('show_ingredient_overview')" v-model="editingObj.showIngredientOverview"></v-checkbox>
            ...
            <model-select
              mode="tags"
              model="User"
              :label="$t('Private_Recipe')"
              :hint="$t('Private_Recipe_Help')"
              persistent-hint
              v-model="editingObj.shared"
              append-to-body
              v-if="editingObj._private"
            ></model-select>
          </v-form>
        </v-tabs-window-item>
      </v-tabs-window>
