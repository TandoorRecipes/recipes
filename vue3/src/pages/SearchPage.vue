<template>
  <v-container class="search">
    ...
    <v-row class="search__advanced" dense>
      <v-col>
            ...
            <v-expansion-panel-text>
              <v-form :disabled="loading" class="mt-4">
                <div v-for="filter in Object.values(filters)" :key="filter.id">
                  <template v-if="filter.enabled">
                    <component :="filter" :is="filter.is" density="compact" v-model="filter.modelValue">
                      <template #append>
                        <v-btn icon="fa-solid fa-times" size="small" variant="plain" @click=";(filter.enabled = false), (filter.modelValue = filter.default)"></v-btn>
                      </template>
                    </component>
                  </template>
                </div>
                ...
                <v-autocomplete
                  ...
                  @update:model-value="(item:string) =>{ filters[item].enabled = true; nextTick(() => {addFilterSelect = null})}"
                  density="compact"
                  :label="$t('AddFilter')"
                  v-model="addFilterSelect"
                ></v-autocomplete>

                <model-select model="CustomFilter" v-model="selectedCustomFilter" density="compact">
                  <template #append>
                    <v-btn icon="fa-solid fa-upload" color="warning" :disabled="selectedCustomFilter == null" @click="loadSelectedCustomFilter()"></v-btn>
              ...
              <v-row>
                <v-col cols="6">
                  <v-select
                    :label="$t('View')"
                    v-model="useUserPreferenceStore().deviceSettings.search_viewMode"
                    :items="[
                      { title: $t('Table'), value: 'table' },
                      { title: $t('Cards'), value: 'grid' },
                    ]"
                    density="compact"
                  ></v-select>
                </v-col>
                <v-col cols="6">
                  <v-select class="float-right" :label="$t('PerPage')" v-model="pageSize" :items="[10, 25, 50, 100]" density="compact" width="100%"></v-select>
                </v-col>
              </v-row>
            </v-expansion-panel-text>
    ...
    <v-row class="search__table" v-if="recipes.length > 0 && useUserPreferenceStore().deviceSettings.search_viewMode == 'table'">
      <v-col>
        <v-card>
          <v-data-table-server
    ...
    <template v-if="recipes.length > 0 && useUserPreferenceStore().deviceSettings.search_viewMode == 'grid'">
      <v-row class="search__grid">
        <v-col cols="6" md="4" v-for="r in recipes" :key="r.id" class="pa-0">
          <recipe-card class="search__recipe" :recipe="r"></recipe-card>
        </v-col>
      </v-row>
    </template>
    <v-row class="search__pagination">
      <v-col cols="12" md="6" offset-md="3" class="text-center">
        <v-pagination
          v-model="page"
          :length="Math.ceil(tableItemCount / pageSize)"
          @update:modelValue="searchRecipes({ page: page })"
          class="ms-2 me-2"
          size="small"
...
function searchRecipes(options: VDataTableUpdateOptions) {
  ...
  api
    .apiRecipeList(searchParameters)
    .then((r) => {
...
function saveCustomFilter() {
  ...
  if (selectedCustomFilter.value != null) {
    ...
    api
      .apiCustomFilterUpdate({ id: selectedCustomFilter.value.id!, customFilter: selectedCustomFilter.value })
      .then((r) => {
...
function createCustomFilter() {
  ...
  loading.value = true
  api
    .apiCustomFilterCreate({ customFilter: { name: newFilterName.value, search: JSON.stringify(filtersToCustomFilterFormat()) } as CustomFilter })
    .then((r) => {
      selectedCustomFilter.value = r
    })
    .catch((err) => {
