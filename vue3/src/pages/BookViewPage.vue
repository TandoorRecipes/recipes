    ...
    <v-row class="book__header">
      <v-col>
        <v-card>
          ...
          <v-card-text class="book__users" v-if="book.shared && book.shared.length > 0">
            <v-chip-group>
              <v-label class="me-2">{{ $t("shared_with") }}</v-label>
              <v-chip v-for="u in book.shared">{{ u.displayName }}</v-chip>
            </v-chip-group>
          </v-card-text>
          <v-card-text class="text-disabled book__description">
            {{ book.description }}
          </v-card-text>
          <v-expansion-panels class="book__contents" v-model="toc">
            <v-expansion-panel>
          ...
          <v-window-item v-for="(entry, i) in recipes" :key="entry.id">
            <v-row>
              <v-col cols="12" md="6">
                ...
                <div class="text-center mt-1 book__recipe">
              ...
              <v-col cols="6" v-if="mdAndUp && recipes.length > i + 1">
                ...
                <div class="text-center mt-1 book__recipenumber">
...
function loadBook() {
  ...
  api
    .apiRecipeBookRetrieve({ id: props.bookId })
    .then((r) => {
      book.value = r
...
function recLoadEntries(page: number) {
  ...
  api
    .apiRecipeBookEntryList({ book: props.bookId, page: page, pageSize: 50 })
    .then((r) => {
...
function recLoadFilter(filterId: number, page: number) {
  ...
  api
    .apiRecipeList({ filter: filterId, page: page, pageSize: 50 })
    .then((r) => {
