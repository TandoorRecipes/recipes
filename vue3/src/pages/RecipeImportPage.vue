<template>
            ...
            <v-stepper-window>
              ...
              <v-stepper-window-item value="url">
                ...
                <div v-if="importType == 'ai'">
                  ...
                  <v-textarea v-model="sourceImportText" :loading="loading" autofocus v-if="aiMode == 'text'" @keydown.enter="loadRecipeFromAiImport()"></v-textarea>
                </div>
                ...
                <v-textarea
                  ...
                  label="JSON/HTML"
                  :loading="loading"
                  v-if="importType == 'source'"
                  :hint="$t('SourceImportHelp')"
                  persistent-hint
                  autofocus
                  @keydown.enter="loadRecipeFromUrl({ data: sourceImportText })"
                ></v-textarea>

                <v-alert v-if="importResponse.error" :title="$t('Error')" :text="importResponse.msg" color="warning"> </v-alert>
                ...
                <v-stepper-actions>
                  ...
                  <template #next>
                    ...
                    <v-btn @click="loadRecipeFromUrl({ data: sourceImportText })" v-if="importType == 'source'" :disabled="sourceImportText == ''" :loading="loading"
                      >{{ $t("Load") }}
                    </v-btn>
                    <v-btn
                      @click="loadRecipeFromAiImport()"
                      v-if="importType == 'ai'"
                      :disabled="(aiMode == 'file' && image == null) || (aiMode == 'text' && sourceImportText == '')"
                      :loading="loading"
                      >{{ $t("Load") }}
                    </v-btn>
                  </template>
              ...
              <v-stepper-window-item value="duplicates">
                <v-alert variant="tonal" v-if="importResponse.duplicates && importResponse.duplicates.length > 0">
                  ...
                  <v-list>
                    <v-list-item :to="{ name: 'RecipeViewPage', params: { id: r.id } }" v-for="r in importResponse.duplicates" :key="r.id">
                      {{ r.name }} (#{{ r.id }})
                    </v-list-item>
                  </v-list>
                </v-alert>
              ...
              <v-stepper-window-item value="keywords_chooser">
                    ...
                    <model-select model="Keyword" v-model="keywordSelect">
                      <template #append>
                        <v-btn
                          ...
                          color="success"
                          @click="
                            keywordSelect.importKeyword = true
                            importResponse.recipe.keywords.push(keywordSelect)
                            keywordSelect = null
                          "
                          :disabled="keywordSelect == null"
                        ></v-btn>
                      </template>
                    </model-select>
                  </v-col>
                ...
                <v-list>
                  <v-list-item border v-for="k in importResponse.recipe.keywords" :key="k" :class="{ 'bg-success': k.importKeyword }" @click="k.importKeyword = !k.importKeyword">
                    {{ k.label }}
                    <template #append>
              ...
              <v-stepper-window-item value="step_editor">
                <v-row>
                  <v-col class="text-center">
                    <v-btn-group border divided>
                      <v-btn prepend-icon="fa-solid fa-shuffle" @click="autoSortIngredients()"
                        ><span v-if="!mobile">{{ $t("Auto_Sort") }}</span></v-btn
                      >
                      ...
                      <v-btn prepend-icon="fa-solid fa-minimize" @click="handleMergeAllSteps()"
                        ><span v-if="!mobile">{{ $t("Merge") }}</span></v-btn
                      >
                    </v-btn-group>
                  </v-col>
                </v-row>
                ...
                <v-card :loading="loading || fileApiLoading">
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" md="6">
                      ...
                      <v-textarea :label="$t('Description')" v-model="importResponse.recipe.description" :rules="[['maxLength', 512]]" counter clearable></v-textarea>
              ...
              <v-stepper-window-item value="app">
                <v-row>
                  <v-col cols="12" md="3" v-for="i in INTEGRATIONS">
                    <v-card
                      prepend-icon="fa-solid fa-carrot"
                      :title="i.name"
                      @click="importApp = i.id"
                      variant="outlined"
                      elevation="1"
              ...
              <v-stepper-window-item value="import_log">
                ...
                <v-stepper-actions>
                  ...
                  <template #next>
                    <v-btn :to="{ name: 'SearchPage', query: { keywords: appImportLog.keyword.id } }" v-if="appImportLog && !appImportLog.running" :disabled="false"
                      >{{ $t("View_Recipes") }}
                    </v-btn>
                  </template>
                </v-stepper-actions>
              ...
              <v-stepper-window-item value="url_list_input">
                <v-textarea :hint="$t('one_url_per_line')" auto-grow max-rows="20" persistent-hint v-model="urlListImportInput"> </v-textarea>
                ...
                <v-stepper-actions>
                  ...
                  <template #next>
                    <v-btn
                      @click="
                        stepper = 'url_list_import'
                        doListImport()
                      "
                      :disabled="urlListImportInput.length == 0"
                      >{{ $t("Import") }}</v-btn
                    >
                  </template>
                </v-stepper-actions>
              ...
              <v-stepper-window-item value="url_list_import">
                ...
                <v-list>
                  <v-list-item
                    border
                    v-for="r in urlListImportedRecipes"
                    :title="r.name"
                    :subtitle="r.sourceUrl"
                    :key="r.id"
                    :to="{ name: 'RecipeViewPage', params: { id: r.id } }"
                    target="_blank"
                  >
                  </v-list-item>
                </v-list>
  ...
  <step-ingredient-sorter-dialog
    :step-index="editingStepIndex"
    :step="editingStep"
    :recipe="importResponse.recipe"
    v-model="dialogIngredientSorter"
    :ingredient-index="editingIngredientIndex"
  ></step-ingredient-sorter-dialog>
</template>
...
function importFromUrlList() {
  ...
  if (url != undefined && url.trim() != "") {
    api
      .apiRecipeFromSourceCreate({ recipeFromSource: { url: url } })
      .then((sourceResponse) => {
        if (sourceResponse.recipe) {
          api
            .apiRecipeCreate({ recipe: sourceResponse.recipe })
            .then((recipe) => {
...
function loadRecipeFromUrl(recipeFromSourceRequest: RecipeFromSource) {
  ...
  api
    .apiRecipeFromSourceCreate({ recipeFromSource: recipeFromSourceRequest })
    .then((r) => {
      if (r.recipeId != null) {
...
function recLoadImportLog(importLogId: number) {
  ...
  api
    .apiImportLogRetrieve({ id: importLogId })
    .then((r) => {
...
function createRecipeFromImport() {
  ...
  if (importResponse.value.recipe) {
    ...
    api
      .apiRecipeCreate({ recipe: importResponse.value.recipe })
      .then((recipe) => {
...
function deleteStep(step: SourceImportStep) {
  if (importResponse.value.recipe) {
    importResponse.value.recipe.steps.splice(
      importResponse.value.recipe.steps.findIndex((x) => x === step),
      1
    )
  }
}
