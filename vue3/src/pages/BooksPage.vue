    ...
    <v-row class="books__list">
      <v-col cols="12" md="3" v-for="(b, i) in books">
        <v-card>
          ...
          <v-card-subtitle class="book__createdby">{{ b.createdBy.displayName }}</v-card-subtitle>
          <v-card-text class="book__description">
            {{ b.description }}
          </v-card-text>
          <v-card-actions class="book__actions">
            <v-btn>
              {{ $t("Edit") }}
              <model-edit-dialog
                model="RecipeBook"
                :item="books[i]"
                @delete="(arg: RecipeBook) => { books.splice(books.findIndex((value: RecipeBook) => value.id == arg.id!),1)}"
              ></model-edit-dialog>
            </v-btn>
