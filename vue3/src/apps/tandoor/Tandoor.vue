<template>
  <v-app>
    <v-app-bar color="tandoor" flat density="comfortable" v-if="!useUserPreferenceStore().isAuthenticated"> </v-app-bar>
    <v-app-bar
      id="header"
      flat
      density="comfortable"
      v-if="useUserPreferenceStore().isAuthenticated"
      :scroll-behavior="useUserPreferenceStore().userSettings.navSticky ? '' : 'hide'"
    >
      <router-link :to="{ name: 'StartPage', params: {} }">
        <v-img
          ...
          width="140px"
          class="ms-2"
          v-if="useUserPreferenceStore().userSettings.navShowLogo && !useUserPreferenceStore().activeSpace.navLogo"
        ></v-img>
        <v-img
          :src="useUserPreferenceStore().activeSpace.navLogo.preview"
          width="140px"
          class="ms-2"
          v-if="useUserPreferenceStore().userSettings.navShowLogo && useUserPreferenceStore().activeSpace.navLogo != undefined"
        ></v-img>
      </router-link>
    ...
    <v-navigation-drawer id="navigation" v-if="lgAndUp && useUserPreferenceStore().isAuthenticated && !isPrintMode">
      <v-list nav>
    ...
    <v-snackbar-queued :vertical="true" location="top center"></v-snackbar-queued>
  </v-app>
</template>
...
import { nextTick, onMounted, computed, watchEffect } from "vue"
...
/**
 * Set CSS variable for header color to enable dynamic theming
 */

const store = useUserPreferenceStore()
const headerColor = computed(() => (store.activeSpace.navBgColor ? store.activeSpace.navBgColor : store.userSettings.navBgColor))
const headerColorUser = computed(() => store.userSettings.navBgColor)

onMounted(() => {
  watchEffect(() => {
    document.documentElement.style.setProperty("--header-color-space", headerColor.value)
    document.documentElement.style.setProperty("--header-color-user", headerColorUser.value)
  })
})
</script>
...
<style>
#header {
  background-color: var(--header-color-space);
}
