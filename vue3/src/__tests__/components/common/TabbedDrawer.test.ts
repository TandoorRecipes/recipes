/**
 * TabbedDrawer tests:
 *  - unpin keeps the drawer open (disable-resize-watcher) + close-also-unpins
 *  - pinnable gating: the pin button is shown by default but suppressed with
 *    :pinnable="false"
 */
...
describe('TabbedDrawer', () => {
    ...
    describe('pinnable gating', () => {
        it('shows the pin button by default', () => {
            expect(mountDrawer({pinned: false}).find('[aria-label="Pin"]').exists()).toBe(true)
        })

        it('hides the pin button when pinnable is false', () => {
            expect(mountDrawer({pinned: false, pinnable: false}).find('[aria-label="Pin"]').exists()).toBe(false)
        })
    })
})
